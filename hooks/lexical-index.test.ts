...
const originalMinScore = process.env.XYLEX_PLUGIN_LEXICAL_RESULT_MIN_SCORE;

function restoreMinScore(): void {
  if (typeof originalMinScore === "string") {
    process.env.XYLEX_PLUGIN_LEXICAL_RESULT_MIN_SCORE = originalMinScore;
    return;
  }
  delete process.env.XYLEX_PLUGIN_LEXICAL_RESULT_MIN_SCORE;
}
...
describe("lexical index", () => {
  it("test_searchSkills_matches_top_level_and_metadata_retrieval_with_query_expansion", () => {
    process.env.XYLEX_PLUGIN_LEXICAL_RESULT_MIN_SCORE = "0";
