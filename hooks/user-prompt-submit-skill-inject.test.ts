...
const originalSeenSkills = process.env.XYLEX_PLUGIN_SEEN_SKILLS;
const originalContextCompacted = process.env.XYLEX_PLUGIN_CONTEXT_COMPACTED;
const originalDedupMode = process.env.XYLEX_PLUGIN_HOOK_DEDUP;
...
afterEach(() => {
  restoreEnv("XYLEX_PLUGIN_SEEN_SKILLS", originalSeenSkills);
  restoreEnv("XYLEX_PLUGIN_CONTEXT_COMPACTED", originalContextCompacted);
  restoreEnv("XYLEX_PLUGIN_HOOK_DEDUP", originalDedupMode);
...
describe("user prompt seen-skills dedup state", () => {
  it("test_resolvePromptSeenSkillState_merges_file_and_claims_when_session_id_present", () => {
    ...
    process.env.XYLEX_PLUGIN_HOOK_DEDUP = "";
    process.env.XYLEX_PLUGIN_SEEN_SKILLS = "skill-env,shared";
    ...
    expect(process.env.XYLEX_PLUGIN_SEEN_SKILLS).toBe("skill-env,shared");
  });
  ...
  it("test_mergeSeenSkillStatesWithCompactionReset_clears_only_high_priority_skills_across_session_artifacts", () => {
    ...
    process.env.XYLEX_PLUGIN_SEEN_SKILLS = "ai-sdk,low-skill,shared";
    ...
    expect(process.env.XYLEX_PLUGIN_SEEN_SKILLS).toBe("low-skill,shared");
    expect(process.env.XYLEX_PLUGIN_CONTEXT_COMPACTED).toBe("");
...
describe("user prompt cursor compatibility", () => {
  ...
  it("test_formatOutput_returns_cursor_flat_shape_with_continue_and_env", () => {
    ...
    expect(output.env).toEqual({ XYLEX_PLUGIN_SEEN_SKILLS: "ai-sdk" });
