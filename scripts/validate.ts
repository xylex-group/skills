#!/usr/bin/env bun
/**
 * Structural validation for the XYLEX Group skills plugin.
...
async function validateGraphSkillRefs() {
  ...
  if (!(await exists(graphPath))) {
    fail("GRAPH_MISSING", "xylex.md not found", {
      file: "xylex.md",
      hint: "Create xylex.md with ⤳ skill: references",
    });
    return;
  }
...
async function validatePluginJson() {
  ...
  // xylex.md (ecosystem graph + conventions)
  const vercelMd = join(ROOT, "xylex.md");
  if (await exists(vercelMd)) {
    pass("xylex.md exists (ecosystem graph + conventions)");
  }
}
...
async function validateCommandConventions() {
  ...
  if (!(await exists(commandsDir))) {
    warn("COMMANDS_DIR_MISSING", "commands/ directory not found (optional)", {
      file: "commands/",
      hint: "Create a commands/ directory with slash command .md files when you add slash commands",
    });
    return;
  }
  ...
  if (cmdFiles.length === 0) {
    pass("commands/ is empty — no slash commands to validate");
    return;
  }
